...
Errors are deliberately specific: schema violations → `400 invalid_event` with a one-line `"/json/pointer": reason` list; unknown `?from_timestamp` → `400 invalid_from_timestamp`; a query string that won't unescape → `400 invalid_query` rather than a `200` that quietly dropped the parameter it choked on; an empty `?from` / `?event_type` → `400 invalid_from` / `400 invalid_event_type` rather than a `200` that silently joins live or subscribes to nothing; aged-out `Last-Event-ID` → `410 event_aged_out` (matches the `410` in Auth0's OpenAPI).
