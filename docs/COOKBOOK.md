...
| 400 | `invalid_query` | The query string wouldn't parse — usually a stray `%` that isn't a valid escape. On `GET /api/v2/events` one bad pair rejects the whole request, because Go keeps the pairs it could parse and drops the one it couldn't: a `?from` that vanished would join live instead of 410-ing, and a vanished `?event_type` would turn a filtered subscription into a firehose. On `POST /admin0/events/expire` it also covers a parameter other than `before` and a repeated `before`, since omitting `before` means "expire everything" and a typo can't be allowed to look like omission. |
